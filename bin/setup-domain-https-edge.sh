...
Environment variables:
  ENV_FILE        Environment file containing DOMAIN, SERVICE_IP and SERVICE_PORT
...
SERVICE_IP="$(read_dotenv_value SERVICE_IP "$ENV_FILE")"
[[ -n "$SERVICE_IP" ]] || die "SERVICE_IP is missing or empty in $ENV_FILE"
SERVICE_PORT="$(read_dotenv_value SERVICE_PORT "$ENV_FILE")"
SERVICE_PORT="${SERVICE_PORT:-443}"
[[ "$SERVICE_PORT" =~ ^[0-9]+$ ]] || die "SERVICE_PORT in $ENV_FILE must be a number"
((SERVICE_PORT >= 1 && SERVICE_PORT <= 65535)) || die "SERVICE_PORT in $ENV_FILE must be between 1 and 65535"
...
server {
    ...
    location / {
        proxy_pass https://${SERVICE_IP}:${SERVICE_PORT};
